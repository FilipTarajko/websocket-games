...
<template>
  ...
  <div class="flex flex-row mt-4 flex-wrap">
    <div
      class="grid h-120 w-full xs:w-1/2 lg:w-3/12 order-3 xs:order-2 lg:order-1"
        ...
        <div style="height: 100%" v-if="socketStore.gameState">
          ...
          <div style="height: 100%" v-else-if="socketStore.gameState.gameName == 'Drawing'">
            <div class="h-1/6 w-full text-center flex flex-col select-none">
              <div
                v-for="lightnessIterator in 11"
                style="height: 12.5%; width: 100%; display: flex"
              >
                <div v-for="hueIterator in 40" style="height: 100%; width: 2.5%">
                  <div
                    @click="
                      setSelectedColor(
                        `hsl(${hueIterator * 9} 50% ${(11 - lightnessIterator) * 10}%)`
                      )
                    "
            ...
            >
              <div
                ...
                v-for="(field, i) in socketStore.gameState.board"
              >
                <div
                  @click="draw(i)"
      ...
      >
        <div>
          <div v-if="socketStore?.gameState?.playerSpots">
            ...
            <Button
              :disabled="!playerHasGameRole"
              @click="socketStore.sendControl('game/leaveSpot')"
              >leave role</Button
            >
          </div>
          <div class="mt-6">
